...
export async function GET() {
  try {
    ...
    const utmLinks = await db.collection('seoUTM')
      .find({})
      .sort({ createdAt: -1 })
      .toArray();
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, url, source, medium, campaign, term, content, fullUrl } = body;

    if (!name || !source || !medium || !campaign) {
      return NextResponse.json({ error: 'Name, source, medium, and campaign are required' }, { status: 400 });
    }
    ...
    return NextResponse.json({ _id: result.insertedId, ...utmLink }, { status: 201 });
  } catch (error) {
