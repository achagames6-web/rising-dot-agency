...
const linksToCheck = [
  ...
  { url: 'https://facebook.com/risingdot', foundOn: '/', type: 'external' as const },
  { url: 'https://twitter.com/risingdot', foundOn: '/', type: 'external' as const },
  { url: 'https://linkedin.com/company/risingdot', foundOn: '/', type: 'external' as const },
];
...
export async function POST() {
  try {
    ...
    for (const link of linksToCheck) {
      try {
        const urlToCheck = link.type === 'internal' 
          ? `${baseUrl}${link.url}` 
          : link.url;
    ...
    return NextResponse.json({ error: 'Failed to scan for broken links' }, { status: 500 });
  }
}
