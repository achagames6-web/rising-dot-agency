...
const IconZendesk = (props: React.SVGProps<SVGSVGElement>) => (
  <svg {...props} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M11.088 3v13.092L1.2 3h9.888zm1.824 4.908V21L22.8 7.908h-9.888zM1.2 21c0-2.736 2.208-4.956 4.944-4.956S11.088 18.264 11.088 21H1.2zm11.712-13.092c0 2.736 2.208 4.956 4.944 4.956S22.8 10.644 22.8 7.908h-9.888z" fill="#03363D"/>
  </svg>
);

const IconIntercom = (props: React.SVGProps<SVGSVGElement>) => (
  <svg {...props} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm-1.5 6h3v9h-3V6zm-4.5 3h3v6h-3V9zm12 0h3v6h-3V9zm-3-1.5h3v7.5h-3V7.5z" fill="#1F8DED"/>
  </svg>
);

const IconBot = (props: React.SVGProps<SVGSVGElement>) => (
  <svg {...props} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M12 2a2 2 0 012 2c0 .74-.4 1.39-1 1.73V7h1a7 7 0 017 7h1a1 1 0 011 1v3a1 1 0 01-1 1h-1v1a2 2 0 01-2 2H5a2 2 0 01-2-2v-1H2a1 1 0 01-1-1v-3a1 1 0 011-1h1a7 7 0 017-7h1V5.73c-.6-.34-1-.99-1-1.73a2 2 0 012-2zM7.5 13a1.5 1.5 0 100 3 1.5 1.5 0 000-3zm9 0a1.5 1.5 0 100 3 1.5 1.5 0 000-3z" fill="#37AFE1"/>
  </svg>
);
...
export default function ChatbotDevelopmentPage() {
  ...
  const { content: heroContent } = useSiteContent<HeroContent>('services-chatbot', 'hero');
  const { content: videoContent } = useSiteContent<VideoContent>('services-chatbot', 'video');
  const { content: chatDemoContent } = useSiteContent<ChatDemoContent>('services-chatbot', 'chatDemo');
  const { content: learningAnimationContent } = useSiteContent<SectionHeadingContent>('services-chatbot', 'learningAnimation');
  const { content: accuracyChartContent } = useSiteContent<AccuracyChartContent>('services-chatbot', 'accuracyChart');
  const { content: caseStudiesContent } = useSiteContent<CaseStudiesContent>('services-chatbot', 'caseStudies');
  const { content: ctaContent } = useSiteContent<CTAContent>('services-chatbot', 'cta');
  const { content: techStackContent } = useSiteContent<TechStackContent>('services-chatbot', 'techStack');
  ...
  const learningAnimation = learningAnimationContent || defaultLearningAnimationContent;
