...
if (!MONGODB_URI) {
  ...
  console.error('   1. Create a .env.local file with: MONGODB_URI=your-mongodb-connection-string');
  console.error('   2. Or set MONGODB_URI environment variable');
  console.error('   3. Or run: $env:MONGODB_URI="your-connection-string"; node scripts/add-indexes.js\n');
...
async function addIndexes() {
  ...
  try {
    ...
    await db.collection('siteContent').createIndex(
      { page: 1, section: 1 },
    ...
    await db.collection('seoRedirects').createIndex(
      { source: 1 },
      { name: 'source_idx', unique: true }
    );
    await db.collection('seoRedirects').createIndex(
      { enabled: 1 },
      { name: 'enabled_idx' }
    ...
    await db.collection('team').createIndex(
      { order: 1 },
      { name: 'order_idx' }
    );
    await db.collection('team').createIndex(
      { visible: 1, order: 1 },
      { name: 'visible_order_idx' }
    );
