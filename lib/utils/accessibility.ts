...
export function validateAriaLabels(html: string): {
  ...
  for (const selector of interactiveElements) {
    ...
    while ((match = regex.exec(html)) !== null) {
      ...
      const ariaLabelledByMatch = elementHtml.match(/aria-labelledby=["']([^"']+)["']/i);
      
      // Check for text content (for buttons and links)
      // Extract content between opening and closing tags
      const contentMatch = elementHtml.match(new RegExp(`<${tagName}[^>]*>(.*?)<\/${tagName}>`, 'is'));
      ...
      const hasLabel = !!(ariaLabelMatch || ariaLabelledByMatch || hasTextContent || hasAlt);
      ...
      if (!hasLabel) {
        errors.push(`Interactive element <${selector}> missing accessible label`);
      }
    }
  }
