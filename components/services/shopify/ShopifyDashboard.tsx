...
export function ShopifyDashboard({
  ...
  const initialMetrics = propMetrics && propMetrics.length > 0 ? propMetrics : defaultMetrics;
  const monthLabels = propMonthLabels && propMonthLabels.length > 0 ? propMonthLabels : defaultMonthLabels;
  
  const [animatedMetrics, setAnimatedMetrics] = useState(initialMetrics.map(m => ({ ...m, value: 0 })));
  ...
  return (
    <div className="w-full max-w-6xl mx-auto space-y-8">
        ...
        {animatedMetrics.map((metric, index) => (
          ...
          >
            ...
            <div className="flex items-baseline gap-1">
              {metric.unit === '$' && (
                <span className="text-2xl font-bold text-white">{metric.unit}</span>
              )}
              <motion.span
      ...
      {/* Revenue Chart */}
      <motion.div
        className="bg-[#0F172A] rounded-xl p-8 border border-white/10"
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.4 }}
      >
        <h3 className="text-2xl font-bold text-white mb-6">{chartTitle}</h3>
        ...
        <div className="relative h-64">
          ...
          <div className="absolute inset-0 flex items-end justify-between gap-2">
            {chartData.map((value, index) => {
              ...
              const barColor = value > 70 ? '#F97316' : value > 40 ? '#2563EB' : '#F97316';
        ...
        <div className="flex justify-between mt-4 text-white/40 text-sm">
          {monthLabels.map((month, index) => (
            <div key={month} className={index >= chartData.length ? 'opacity-30' : ''}>
              {month}
            </div>
          ))}
        </div>
