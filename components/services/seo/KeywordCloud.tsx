...
export function KeywordCloud({
  ...
  return (
    <div className="relative w-full min-h-[600px] bg-[#0F172A] rounded-2xl p-8 overflow-hidden">
      ...
      {/* Keyword cloud */}
      <motion.div 
        className="relative z-10 flex flex-wrap items-center justify-center gap-4 min-h-[400px]"
        layout
      >
            ...
            >
              <motion.span
                ...
                style={{
                  ...
                  color: color,
                  textShadow: hoveredKeyword === keyword.text 
                    ? `0 0 20px ${color}80` 
                    : 'none'
                }}
                animate={{
              ...
              {hoveredKeyword === keyword.text && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: -10 }}
                >
                  <div className="bg-[#1E293B] px-3 py-1 rounded-lg border border-[#64748B]/30">
      ...
      <div className="relative z-10 mt-8 grid grid-cols-3 gap-4">
        <div className="bg-[#1E293B]/50 rounded-lg p-4 border border-[#64748B]/30">
          <div className="text-[#64748B] text-sm mb-1">{totalKeywordsLabel}</div>
          <div className="text-white text-2xl font-bold">{filteredKeywords.length}</div>
        </div>
        <div className="bg-[#1E293B]/50 rounded-lg p-4 border border-[#64748B]/30">
          <div className="text-[#64748B] text-sm mb-1">{avgImportanceLabel}</div>
          <div className="text-white text-2xl font-bold">
            {(filteredKeywords.reduce((sum, k) => sum + k.importance, 0) / filteredKeywords.length).toFixed(1)}
          </div>
        </div>
        <div className="bg-[#1E293B]/50 rounded-lg p-4 border border-[#64748B]/30">
          <div className="text-[#64748B] text-sm mb-1">{categoriesLabel}</div>
          <div className="text-white text-2xl font-bold">
