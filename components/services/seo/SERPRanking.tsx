...
const defaultKeywords: KeywordRanking[] = [
  { keyword: 'Web Design Agency', startPosition: 10, endPosition: 1, color: '#F97316' },
  { keyword: 'Custom Chatbots', startPosition: 8, endPosition: 2, color: '#F97316' },
  { keyword: 'N8N Automation', startPosition: 12, endPosition: 3, color: '#F97316' },
  { keyword: 'Shopify Development', startPosition: 15, endPosition: 4, color: '#F97316' },
  { keyword: 'WordPress Solutions', startPosition: 9, endPosition: 5, color: '#F97316' }
];
...
export function SERPRanking({
  ...
  return (
    <div className="relative w-full min-h-[550px] bg-[#0F172A] rounded-2xl p-8 overflow-hidden">
      {/* Background grid */}
      ...
      <div className="relative z-10 space-y-3">
        {[...Array(10)].map((_, index) => {
          ...
          const keyword = keywords.find(k => 
            animationStarted ? k.endPosition === position : k.startPosition === position
          );
          ...
          return (
            ...
            >
              <div className={`
                ...
                ${isTopThree ? 'bg-[#F97316]/10 border-[#F97316]' : 'bg-[#1E293B]/50 border-[#64748B]/30'}
              ...
              `}>
                ...
                <div className="flex-1">
                  {keyword && (
                    <motion.div
                      initial={animationStarted ? { y: (keyword.startPosition - keyword.endPosition) * 60 } : {}}
                      animate={{ y: 0 }}
                      transition={{ 
                        duration: 2, 
                        delay: 1,
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className="text-white font-semibold text-lg">{keyword.keyword}</div>
                        <motion.div
                        ...
                        >
                          ...
                          <span>+{keyword.startPosition - keyword.endPosition} positions</span>
                        </motion.div>
                      </div>
                      <div className="text-[#64748B] text-sm mt-1">
                        {siteDomain} › services › {keyword.keyword.toLowerCase().replace(/\s+/g, '-')}
                      </div>
                    </motion.div>
                  )}
                  ...
                  {competitor && !keyword && (
                    <div className="flex items-center gap-3">
                      <div className="text-[#F59E0B] font-medium">{competitor.name}</div>
                      <div className="text-[#64748B] text-sm">competitor-site.com</div>
                    </div>
                  )}
                ...
                {keyword && (
                  <motion.div
                    className="w-2 h-12 rounded-full bg-gradient-to-b from-[#F97316] to-[#2563EB]"
                    initial={{ scaleY: 0 }}
                    animate={{ scaleY: 1 }}
                ...
                {competitor && (
                  <div className="w-2 h-12 rounded-full bg-[#F59E0B]/50" />
                )}
              </div>
