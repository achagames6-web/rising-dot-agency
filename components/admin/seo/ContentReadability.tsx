...
import { RefreshCw, BookOpen, CheckCircle, AlertTriangle, XCircle } from 'lucide-react';
...
export default function ContentReadability() {
  ...
  const analyzeReadability = async () => {
    setAnalyzing(true);
    try {
      const res = await fetch('/api/admin/seo/readability/analyze', { method: 'POST' });
      if (res.ok) {
        const data = await res.json();
        setResults(data);
  ...
  const getGradeDescription = (grade: string) => {
    const gradeMap: Record<string, string> = {
      ...
      '8th grade': 'Plain English. Easily understood by 13- to 15-year-old students.',
      '9th grade': 'Fairly difficult to read.',
      '10th grade': 'Difficult to read.',
      '11th grade': 'Difficult to read.',
      '12th grade': 'Very difficult to read. Best understood by university graduates.',
      'College': 'Very difficult to read. Best understood by university graduates.',
      'College graduate': 'Extremely difficult to read. Best understood by university graduates.',
      'Professional': 'Extremely difficult to read. Best understood by professionals.',
    };
    return gradeMap[grade] || 'Standard readability level.';
  };

  const selectedResult = results.find(r => r.page === selectedPage);
  const avgScore = results.length > 0 ? Math.round(results.reduce((acc, r) => acc + r.fleschScore, 0) / results.length) : 0;
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-[#1E293B] rounded-xl border border-slate-700/50 p-4">
          <p className="text-2xl font-bold text-white">
            {results.length > 0 ? Math.round(results.reduce((acc, r) => acc + r.wordCount, 0) / results.length) : 0}
          </p>
          <p className="text-sm text-slate-400">Avg. Word Count</p>
        </div>
        <div className="bg-[#1E293B] rounded-xl border border-slate-700/50 p-4">
          <p className="text-2xl font-bold text-white">
            {results.length > 0 ? Math.round(results.reduce((acc, r) => acc + r.readingTime, 0) / results.length) : 0} min
          </p>
              ...
              >
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-1">{result.fleschGrade}</p>
                  </div>
                  <span className={`text-lg font-bold ${getScoreColor(result.fleschScore)}`}>
                    {result.fleschScore}
                  </span>
                </div>
              </button>
        ...
        <div className="lg:col-span-2 bg-[#1E293B] rounded-xl border border-slate-700/50 overflow-hidden">
          <div className="p-4 border-b border-slate-700">
            <h3 className="text-white font-semibold">
              {selectedResult ? `Readability: ${selectedResult.page}` : 'Select a page'}
            </h3>
          </div>
          <div className="p-4 max-h-[500px] overflow-y-auto">
            {selectedResult ? (
              <div className="space-y-6">
                {/* Score */}
                <div className={`p-6 rounded-xl border-2 text-center ${getScoreBg(selectedResult.fleschScore)}`}>
                  <p className={`text-5xl font-bold ${getScoreColor(selectedResult.fleschScore)}`}>
                    {selectedResult.fleschScore}
                  </p>
                  <p className="text-white font-medium mt-2">{selectedResult.fleschGrade}</p>
                  <p className="text-sm text-slate-400 mt-1">{getGradeDescription(selectedResult.fleschGrade)}</p>
                </div>

                {/* Metrics */}
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  <div className="p-3 bg-[#0F172A] rounded-lg">
                    <p className="text-2xl font-bold text-white">{selectedResult.wordCount}</p>
                    <p className="text-xs text-slate-400">Words</p>
                  </div>
                  <div className="p-3 bg-[#0F172A] rounded-lg">
                    <p className="text-2xl font-bold text-white">{selectedResult.sentenceCount}</p>
                    <p className="text-xs text-slate-400">Sentences</p>
                  </div>
                  <div className="p-3 bg-[#0F172A] rounded-lg">
                    <p className="text-2xl font-bold text-white">{selectedResult.paragraphCount}</p>
                    <p className="text-xs text-slate-400">Paragraphs</p>
                  </div>
                  <div className="p-3 bg-[#0F172A] rounded-lg">
                    <p className="text-2xl font-bold text-white">{selectedResult.avgSentenceLength.toFixed(1)}</p>
                    <p className="text-xs text-slate-400">Avg. Sentence Length</p>
                  </div>
                  <div className="p-3 bg-[#0F172A] rounded-lg">
                    <p className="text-2xl font-bold text-white">{selectedResult.avgWordLength.toFixed(1)}</p>
                    <p className="text-xs text-slate-400">Avg. Word Length</p>
                  </div>
                  <div className="p-3 bg-[#0F172A] rounded-lg">
                    <p className="text-2xl font-bold text-white">{selectedResult.readingTime} min</p>
                ...
                {selectedResult.issues.length > 0 && (
                  <div className="space-y-2">
                    ...
                    {selectedResult.issues.map((issue, i) => (
                      ...
                      >
                        <AlertTriangle className={`w-4 h-4 flex-shrink-0 mt-0.5 ${issue.type === 'error' ? 'text-red-400' : 'text-amber-400'}`} />
