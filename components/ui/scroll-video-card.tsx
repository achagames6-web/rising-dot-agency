...
export const ScrollVideoCard: React.FC<ScrollVideoCardProps> = ({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const ScrollTriggerPkg = (await import("gsap/ScrollTrigger").catch(() => import("gsap/dist/ScrollTrigger"))) || {};
      ...
      if (overlayContent) gsap.set(overlayContent, { filter: `blur(${overlayBlur}px)`, scale: 1.05, y: 30 });

      mainTl.to(container, { width: target.width, height: target.height, borderRadius: target.borderRadius, ease: "expo.out" }, 0);
      mainTl.to(overlayDarkenEl, { backgroundColor: "rgba(0,0,0,0.4)", ease: "power2.out" }, 0);
      
      if (overlayEl) {
        mainTl.to(overlayEl, { clipPath: "inset(0% 0 0 0)", backdropFilter: `blur(${overlayBlur}px)`, ease: "expo.out" }, overlayRevealDelay);
      }
      if (overlayContent) mainTl.to(overlayContent, { y: 0, filter: "blur(0px)", scale: 1, ease: "expo.out" }, overlayRevealDelay + 0.05);

      const videoEl = container.querySelector("video") as HTMLVideoElement | null;
      if (videoEl) {
        const tryPlay = () => videoEl.play().catch(() => {});
        tryPlay();
        ScrollTrigger.create({ trigger: triggerEl, start: "top center", onEnter: tryPlay });
      }
    })();
  ...
  }, [isClient, initialBoxSize, targetSize, scrollHeightVh, overlayBlur, overlayRevealDelay, sticky, smoothScroll]);
        ...
        <div className={`${sticky ? 'sticky top-0' : ''} h-screen flex items-center justify-center`}>
          ...
          >
            ...
            {overlay && (
              <div
                ref={overlayRef}
                className="absolute inset-0 bg-black/60 backdrop-blur-md flex flex-col items-center justify-center text-center p-8 z-10"
                style={{ clipPath: "inset(100% 0 0 0)" }}
              >
                <div ref={overlayContentRef} className="max-w-3xl space-y-6">
                  {overlay.caption && (
                    <div ref={overlayCaptionRef} className="mb-4">
                      <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#F58122]/20 border border-[#F58122]/30 text-[#F58122] font-semibold text-xs uppercase tracking-widest">
                        <span className="w-2 h-2 rounded-full bg-[#F58122] animate-pulse" />
                        {overlay.caption}
                      </span>
                    </div>
                  )}
                  ...
                  <div className="w-24 h-1 mx-auto rounded-full bg-gradient-to-r from-[#F58122] to-[#37AFE1]" />
                  {overlay.paragraphs?.map((p, i) => (
                    <p key={i} className="text-white/90 text-lg md:text-xl leading-relaxed font-light">{p}</p>
                  ))}
                  {overlay.extra}
                </div>
              </div>
